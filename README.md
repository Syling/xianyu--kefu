...
- **安全保护** - 防暴力破解（IP封禁、用户锁定、动态延迟响应）、会话管理、安全日志
...
- **主动议价** - 支持搜索公开商品、查看详情并向卖家发送议价消息，附带频率限制防封
- **飞书机器人** - 支持飞书机器人回调，通过飞书发送议价消息
- **多渠道通知** - 支持钉钉、飞书、邮件、Telegram、Bark、Webhook、QQ 等多种通知渠道
- **公告系统** - 支持远程 GitHub 拉取和本地管理员自定义公告
...
- **消息流看门狗** - 检测假在线状态（只有心跳无业务包），自动触发重连
...
- **敏感信息加密** - Cookie、密码等敏感数据使用 Fernet 加密存储
- **订单历史同步** - 从闲鱼 API 同步已完成的历史订单数据
- **容器化部署** - 支持 Docker / Docker Compose 持久化部署

### 🛡️ 风控与安全
- **风控熔断** - 滑块连续失败冷静期、密码登录退避、认证失败熔断、账号风控自动禁用
- **远程滑块控制（刮刮乐）** - WebSocket 实时传输浏览器截图，前端远程滑动验证
- **第三方验证码识别** - 集成 YesCaptcha / 2Captcha 服务自动识别
- **滑块轨迹模拟** - Perlin 噪声生成类人鼠标轨迹，集成反检测技术
- **敏感信息加密** - Cookie、代理密码等使用 Fernet 加密存储，自动明文迁移密文
- **防暴力破解** - IP 封禁、用户锁定、图形验证码、动态延迟响应
- **通知告警** - 发现 Token 异常、滑块验证成功、风控事件时多渠道推送通知

### 📦 订单管理
- **订单状态管理** - 完整的状态转换矩阵（processing→pending_ship→shipped→completed→cancelled），支持退款回退
- **订单事件广播** - 进程内订单事件发布/订阅系统，按用户 ID 实时广播
- **订单详情补抓** - 首次超时自动后台补抓，避免整单丢失
- **消息优先级队列** - 0-3级消息优先级，心跳消息最高优先
