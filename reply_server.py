...
class BuyerBargainRequest(BaseModel):
    account_id: str = ""
    item_title: str
    item_url: str = ""
    seller_user_id: str = ""
...
class BuyerBargainGenerateRequest(BaseModel):
    item_title: str
    original_price: Optional[float] = None
    target_price: Optional[float] = None
    note: str = ""
...
class BuyerBargainItemSendRequest(BaseModel):
    account_id: str
    item_id: str
    item_title: str = ""
    item_url: str = ""
...
class BuyerBargainChatIdRequest(BaseModel):
    chat_id: str


class BuyerBargainReplyRequest(BaseModel):
    content: str
...
BUYER_BARGAIN_COLUMNS = [
    'id', 'user_id', 'account_id', 'item_title', 'item_url', 'seller_user_id',
    'chat_id', 'original_price', 'target_price', 'message', 'status',
    'send_result', 'has_reply', 'reply_content', 'replied_at',
    'created_at', 'updated_at'
]


def _ensure_buyer_bargain_table():
    with db_manager.lock:
        cursor = db_manager.conn.cursor()
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS buyer_bargains (
        ...
        """)
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_buyer_bargains_user_id ON buyer_bargains(user_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_buyer_bargains_chat_id ON buyer_bargains(chat_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_buyer_bargains_status_chat ON buyer_bargains(status, chat_id)")
        for col_name, col_type in (('has_reply', 'INTEGER DEFAULT 0'), ('reply_content', 'TEXT'), ('replied_at', 'TEXT')):
            try:
                cursor.execute(f"ALTER TABLE buyer_bargains ADD COLUMN {col_name} {col_type}")
            except Exception:
                pass
        db_manager.conn.commit()


def _buyer_bargain_row_to_dict(row) -> Dict[str, Any]:
    return dict(zip(BUYER_BARGAIN_COLUMNS, row))


def _build_buyer_bargain_message(item_title: str, original_price: Optional[float], target_price: Optional[float], note: str = "") -> str:
    title = str(item_title or '').strip() or '这个商品'
    note = str(note or '').strip()

    price_part = ''
    if target_price:
        price_part = f"{target_price:g}元可以吗？"
    elif original_price:
        suggested_price = max(round(float(original_price) * 0.85, 2), 1)
        price_part = f"{suggested_price:g}元可以吗？"
    else:
        price_part = "价格还能优惠一点吗？"
...
@app.post('/buyer-bargains/generate')
def generate_buyer_bargain_message(request: BuyerBargainGenerateRequest, current_user: Dict[str, Any] = Depends(get_current_user)):
    return {
        'success': True,
        'message': _build_buyer_bargain_message(
            request.item_title,
...
@app.get('/buyer-bargains')
def list_buyer_bargains(current_user: Dict[str, Any] = Depends(get_current_user)):
    _ensure_buyer_bargain_table()
    rows = db_manager.execute_query(
        f"SELECT {', '.join(BUYER_BARGAIN_COLUMNS)} FROM buyer_bargains WHERE user_id = ? ORDER BY id DESC",
        (current_user['user_id'],)
    )
    return {
        'success': True,
        'items': [_buyer_bargain_row_to_dict(row) for row in rows]
    }


@app.post('/buyer-bargains')
def create_buyer_bargain(request: BuyerBargainRequest, current_user: Dict[str, Any] = Depends(get_current_user)):
    _ensure_buyer_bargain_table()
    item_title = str(request.item_title or '').strip()
    if not item_title:
        raise HTTPException(status_code=400, detail='商品标题不能为空')

    account_id = str(request.account_id or '').strip()
    if account_id:
        account_id = _ensure_cookie_access(account_id, current_user)

    message = str(request.message or '').strip() or _build_buyer_bargain_message(
        item_title,
        request.original_price,
        request.target_price,
    )
    ...
    with db_manager.lock:
        cursor = db_manager.conn.cursor()
        cursor.execute("""
            INSERT INTO buyer_bargains (
                user_id, account_id, item_title, item_url, seller_user_id, chat_id,
                original_price, target_price, message, status, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            current_user['user_id'],
            account_id,
            item_title,
            str(request.item_url or '').strip(),
            str(request.seller_user_id or '').strip(),
        ...
        bargain_id = cursor.lastrowid
        db_manager.conn.commit()

    return {'success': True, 'id': bargain_id, 'message': '议价任务已保存'}


@app.post('/buyer-bargains/{bargain_id}/send')
async def send_buyer_bargain(bargain_id: int, current_user: Dict[str, Any] = Depends(get_current_user)):
    _ensure_buyer_bargain_table()
    rows = db_manager.execute_query(
        f"SELECT {', '.join(BUYER_BARGAIN_COLUMNS)} FROM buyer_bargains WHERE id = ? AND user_id = ?",
        (bargain_id, current_user['user_id'])
    )
    if not rows:
        raise HTTPException(status_code=404, detail='议价任务不存在')

    bargain = _buyer_bargain_row_to_dict(rows[0])
    account_id = str(bargain.get('account_id') or '').strip()
    chat_id = str(bargain.get('chat_id') or '').strip()
    seller_user_id = str(bargain.get('seller_user_id') or '').strip()
    message = str(bargain.get('message') or '').strip()

    if not account_id:
        raise HTTPException(status_code=400, detail='请先选择发送账号')
    if not chat_id or not seller_user_id:
        raise HTTPException(status_code=400, detail='需要填写聊天ID和卖家用户ID后才能直接发送')
    account_id = _ensure_cookie_access(account_id, current_user)

    from XianyuAutoAsync import XianyuLive, ConnectionState
    live_instance = XianyuLive.get_instance(account_id)
    if not live_instance or live_instance.connection_state != ConnectionState.CONNECTED or not live_instance.ws:
        raise HTTPException(status_code=400, detail='账号后台未连接，请先在后台管理启动账号')

    await _run_live_instance_on_manager_loop(
        account_id,
        lambda: live_instance.send_msg(live_instance.ws, chat_id, seller_user_id, message),
        timeout=15,
    )
    ...
    with db_manager.lock:
        cursor = db_manager.conn.cursor()
        cursor.execute(
            "UPDATE buyer_bargains SET status = ?, send_result = ?, updated_at = ? WHERE id = ? AND user_id = ?",
            ('sent', '发送成功', datetime.now().strftime('%Y-%m-%d %H:%M:%S'), bargain_id, current_user['user_id'])
        )
        db_manager.conn.commit()

    return {'success': True, 'message': '议价消息已发送'}


@app.post('/buyer-bargains/send-item')
async def send_buyer_bargain_to_item(request: BuyerBargainItemSendRequest, current_user: Dict[str, Any] = Depends(get_current_user)):
    """按公开商品ID发送主动议价消息。

    这里复用现有闲鱼实例的商品详情解析和单次消息发送逻辑：
    1. 通过商品ID获取卖家ID，避免让用户手动填写卖家ID/聊天ID。
    2. 发送前由工具函数模拟阅读和输入延迟，尽量贴近正常用户主动询问。
    3. 发送成功后回填 chat_id，后续卖家回复时可关联到议价记录。
    """
    account_id = str(request.account_id or '').strip()
    item_id = str(request.item_id or '').strip()
    bargain_message = str(request.message or '').strip()

    if not account_id:
        raise HTTPException(status_code=400, detail='请选择发送账号')
    if not item_id:
        raise HTTPException(status_code=400, detail='商品ID不能为空')
    if not bargain_message:
        raise HTTPException(status_code=400, detail='议价内容不能为空')
    ...
    if not instance:
        raise HTTPException(status_code=400, detail='议价账号未运行，请先在「后台管理」启动该账号')

    from utils.feishu_event_handler import get_item_seller_info, send_bargain_message
    from utils.feishu_event_handler import _bargain_rate_limiter

    can_send, limit_reason = _bargain_rate_limiter.can_send(account_id)
    if not can_send:
        raise HTTPException(status_code=429, detail=f'发送太频繁，{limit_reason}')

    async def _send_from_account_loop():
        seller_info = await get_item_seller_info(instance, item_id)
        if not seller_info:
            return False, '无法获取商品卖家信息，请确认商品链接有效'

        seller_id = seller_info.get('seller_id')
        if seller_id and str(seller_id) == str(getattr(instance, 'myid', '')):
            return False, '不能给自己的商品发送议价消息'

        success, chat_id = await send_bargain_message(instance, str(seller_id), item_id, bargain_message)
        if not success:
            return False, '议价消息发送失败，请检查账号连接状态'

        return True, {
            'seller_id': str(seller_id),
            'seller_name': seller_info.get('seller_name') or request.seller_name or '卖家',
            'title': seller_info.get('title') or request.item_title or '未知商品',
            'price': seller_info.get('price') or request.price or '',
            'chat_id': chat_id or '',
        }

    success, result = await _run_live_instance_on_manager_loop(
        account_id,
        _send_from_account_loop,
        timeout=90,
    )
    if not success:
    ...
    with db_manager.lock:
        ...
        cursor.execute("""
            INSERT INTO buyer_bargains (
                user_id, account_id, item_title, item_url, seller_user_id, chat_id,
                original_price, target_price, message, status, send_result, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            current_user['user_id'],
            account_id,
            request.item_title or result.get('title') or '',
            request.item_url,
            result.get('seller_id') or '',
            result.get('chat_id') or '',
            None,
            None,
            bargain_message,
            'sent',
            f"发送成功，卖家：{result.get('seller_name') or '卖家'}",
            now,
            now,
        ))
        bargain_id = cursor.lastrowid
...
@app.delete('/buyer-bargains/{bargain_id}')
def delete_buyer_bargain(bargain_id: int, current_user: Dict[str, Any] = Depends(get_current_user)):
    _ensure_buyer_bargain_table()
    with db_manager.lock:
        cursor = db_manager.conn.cursor()
        cursor.execute("DELETE FROM buyer_bargains WHERE id = ? AND user_id = ?", (bargain_id, current_user['user_id']))
        deleted = cursor.rowcount
        db_manager.conn.commit()
    if not deleted:
        raise HTTPException(status_code=404, detail='议价任务不存在')
    return {'success': True, 'message': '议价任务已删除'}


@app.post('/buyer-bargains/{bargain_id}/chat-id')
def update_buyer_bargain_chat_id(bargain_id: int, request: BuyerBargainChatIdRequest, current_user: Dict[str, Any] = Depends(get_current_user)):
    """发送议价后回填 chat_id"""
    _ensure_buyer_bargain_table()
    now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    ...
    with db_manager.lock:
        cursor = db_manager.conn.cursor()
        cursor.execute(
            "UPDATE buyer_bargains SET chat_id = ?, updated_at = ? WHERE id = ? AND user_id = ?",
            (chat_id, now, bargain_id, current_user['user_id'])
        )
        updated = cursor.rowcount
        db_manager.conn.commit()
    if not updated:
        raise HTTPException(status_code=404, detail='议价记录不存在')
    return {'success': True, 'message': '聊天ID已更新'}


@app.post('/buyer-bargains/{bargain_id}/reply')
def mark_buyer_bargain_reply(bargain_id: int, request: BuyerBargainReplyRequest, current_user: Dict[str, Any] = Depends(get_current_user)):
    """卖家回复时标记议价记录"""
    _ensure_buyer_bargain_table()
    now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    with db_manager.lock:
        cursor = db_manager.conn.cursor()
        cursor.execute(
            "UPDATE buyer_bargains SET has_reply = 1, reply_content = ?, replied_at = ?, updated_at = ? WHERE id = ? AND user_id = ?",
            (str(request.content or ''), now, now, bargain_id, current_user['user_id'])
        )
        updated = cursor.rowcount
        db_manager.conn.commit()
    if not updated:
        raise HTTPException(status_code=404, detail='议价记录不存在')
    return {'success': True, 'message': '卖家回复已标记'}


@app.get('/buyer-bargains/unreplied')
def list_unreplied_buyer_bargains(current_user: Dict[str, Any] = Depends(get_current_user)):
    """列出已发送但卖家尚未回复的议价记录"""
    _ensure_buyer_bargain_table()
    rows = db_manager.execute_query(
        f"SELECT {', '.join(BUYER_BARGAIN_COLUMNS)} FROM buyer_bargains WHERE user_id = ? AND status = 'sent' AND (has_reply IS NULL OR has_reply = 0) AND chat_id != '' ORDER BY id DESC",
        (current_user['user_id'],)
    )
    return {'success': True, 'items': [_buyer_bargain_row_to_dict(row) for row in rows]}
...
def _get_background_runtime_entry(cookie_id: str, cookie_value: str) -> Dict[str, Any]:
    manager = cookie_manager.manager
    details = db_manager.get_cookie_details(cookie_id) or {}
    runtime_status = _build_live_runtime_status(cookie_id)
    task = getattr(manager, 'tasks', {}).get(cookie_id) if manager else None
    task_running = bool(task and not task.done())
    instance_running = bool(runtime_status.get('running') or runtime_status.get('instance_exists'))
    enabled = manager.get_cookie_status(cookie_id) if manager else False
    ...
    return {
        ...
        'enabled': enabled,
        'running': task_running or instance_running,
        'runtime_status': runtime_status,
    }


def _stop_background_cookie(cookie_id: str) -> bool:
    manager = cookie_manager.manager
    if manager is None:
        raise HTTPException(status_code=500, detail='CookieManager is not ready')

    stopped = False
    if manager.get_cookie_status(cookie_id):
        manager.update_cookie_status(cookie_id, False)
        stopped = True

    task = getattr(manager, 'tasks', {}).get(cookie_id)
    if task and not task.done() and hasattr(manager, '_stop_cookie_task'):
        manager._stop_cookie_task(cookie_id)
        stopped = True
...
@app.get("/backgrounds")
def list_backgrounds(current_user: Dict[str, Any] = Depends(get_current_user)):
    manager = cookie_manager.manager
    user_cookies = _get_user_cookies_map(current_user)

    accounts = [
        _get_background_runtime_entry(cookie_id, cookie_value)
        for cookie_id, cookie_value in user_cookies.items()
    ]
    running_accounts = [account for account in accounts if account.get('running')]

    return {
        'success': True,
        'accounts': accounts,
        'active_cookie_id': running_accounts[0]['id'] if running_accounts else '',
...
def activate_background(cid: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    for other_cookie_id in user_cookies.keys():
        if other_cookie_id != cid:
            _stop_background_cookie(other_cookie_id)
    ...
    if not manager.get_cookie_status(cid):
        ...
        manager.update_cookie(cid, cookie_value, save_to_db=False)

    return {
        'success': True,
        'message': f'已启动账号 "{cid}" 后台',
        'active_cookie_id': cid,
    }
...
def stop_all_backgrounds(current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    stopped_count = 0
    for cookie_id in _get_user_cookies_map(current_user).keys():
        if _stop_background_cookie(cookie_id):
            stopped_count += 1
...
def _build_card_test_result(card: Dict[str, Any]) -> Dict[str, Any]:
    card_type = card.get('type')
    preview = ''
    warnings = []

    if card_type == 'text':
        preview = card.get('text_content') or ''
        if not preview.strip():
            warnings.append('固定文字内容为空')
    elif card_type == 'data':
        preview = db_manager.peek_batch_data(card.get('id')) or ''
        if not preview:
            warnings.append('批量数据为空，没有可发货的卡密/数据')
    elif card_type == 'image':
        preview = card.get('image_url') or ''
        if not preview:
            warnings.append('图片卡券没有图片地址')
    elif card_type == 'api':
        api_config = card.get('api_config') or {}
        if isinstance(api_config, str):
            try:
                api_config = json.loads(api_config)
            except Exception:
                api_config = {}
        preview = api_config.get('url') or ''
        if not preview:
            warnings.append('API卡券没有配置请求地址')
    elif card_type == 'yifan_api':
        api_config = card.get('api_config') or {}
        if isinstance(api_config, str):
            try:
                api_config = json.loads(api_config)
        ...
        preview = f"goods_id={api_config.get('goods_id') or ''}"
        if missing:
            warnings.append(f"亦凡API缺少配置：{', '.join(missing)}")
    else:
        warnings.append('未知卡券类型')
...
def test_card(card_id: int, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        return _build_card_test_result(card)
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def create_delivery_rule(rule_data: dict, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        if not keyword:
            raise HTTPException(status_code=400, detail="商品关键字不能为空")
        if card_id is None:
            raise HTTPException(status_code=400, detail="请选择卡券")
...
def test_delivery_rule(rule_id: int, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        card = db_manager.get_card_by_id(rule.get('card_id'), user_id)
        if not card:
            return {
                'success': False,
                'rule_id': rule_id,
                'message': '发货规则关联的卡券不存在，请重新选择卡券',
                'warnings': ['关联卡券不存在'],
            }

        card_test = _build_card_test_result(card)
        keyword = str(rule.get('keyword') or '').strip()
        warnings = list(card_test.get('warnings') or [])
        if not keyword:
            warnings.append('商品关键字为空')
        if not rule.get('enabled'):
            warnings.append('发货规则已禁用')
        if not card.get('enabled'):
            warnings.append('关联卡券已禁用')
        ...
        return {
            ...
            'card_name': card.get('name'),
            'card_type': card.get('type'),
            'delivery_count': rule.get('delivery_count') or 1,
            'preview': card_test.get('preview') or '',
            'warnings': warnings,
            'message': '发货规则配置可用' if not warnings else '发货规则配置需要检查',
        }
    except HTTPException:
        raise
    except Exception as e:
...
def delete_card(card_id: int, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        for rule in db_manager.get_all_delivery_rules(user_id):
            if rule.get('card_id') == card_id:
                db_manager.delete_delivery_rule(rule.get('id'), user_id)
