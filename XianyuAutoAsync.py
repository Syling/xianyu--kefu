...
class XianyuLive:
    ...
    # 滑块连续失败计数器与冷静期
    _slider_consecutive_failures = {}  # {cookie_id: int}
    _slider_cooldown_until = {}  # {cookie_id: float}
    _slider_cooldown_threshold = 3  # 连续失败3次触发冷静期
    _slider_cooldown_duration = 1800  # 冷静期时长：30分钟

    # 手动刷新状态：用于避免手动刷新与自动滑块/自动Cookie刷新互相踩踏
    _manual_refresh_state = {}  # {cookie_id: {'source': str, 'phase': str, 'started_at': float, 'expires_at': Optional[float], 'previous_cookie_refresh_enabled': Optional[bool]}}
    _manual_refresh_lock = threading.Lock()
    _manual_refresh_handoff_ttl = 120  # 刷新交接恢复窗口（秒）
    _manual_refresh_ttl = 300  # 手动刷新保护期最长 5 分钟，超时自动释放
    ...
    def _cleanup_manual_refresh_state(cls):
        """清理过期的手动刷新或交接恢复状态（超时自动释放锁）。"""
        now = time.time()
        expired_cookie_ids = []
        recovered_instances = []
        with cls._manual_refresh_lock:
            for cookie_id, state in dict(cls._manual_refresh_state).items():
                expires_at = state.get('expires_at', 0)
                if expires_at and now > expires_at:
                    expired_cookie_ids.append((cookie_id, state.get('previous_cookie_refresh_enabled')))
                    cls._manual_refresh_state.pop(cookie_id, None)

        for cookie_id, previous_enabled in expired_cookie_ids:
            logger.warning(f"【{cookie_id}】手动刷新保护期已超时（{cls._manual_refresh_ttl}秒），自动释放锁")
            live_instance = cls.get_instance(cookie_id)
            if live_instance is not None and previous_enabled is not None:
                live_instance.enable_cookie_refresh(previous_enabled)
                if previous_enabled:
                    live_instance.last_cookie_refresh_time = time.time()
                recovered_instances.append(cookie_id)
                logger.warning(f"【{cookie_id}】已恢复自动Cookie刷新: {previous_enabled}")
    ...
    def classify_password_login_failure(error_message: str) -> Tuple[str, int]:
        ...
        if any(keyword in message for keyword in ["二维码验证超时", "等待二维码验证超时"]):
            return "qr_verify_timeout", 1800
        if any(
    ...
    @classmethod
    def record_slider_failure(cls, cookie_id: str) -> bool:
        """记录一次滑块验证失败，返回是否触发了冷静期"""
        if not cookie_id:
            return False
        cls._slider_consecutive_failures[cookie_id] = cls._slider_consecutive_failures.get(cookie_id, 0) + 1
        count = cls._slider_consecutive_failures[cookie_id]
        logger.warning(f"【{cookie_id}】滑块连续失败计数: {count}/{cls._slider_cooldown_threshold}")
        if count >= cls._slider_cooldown_threshold:
            cooldown_until = time.time() + cls._slider_cooldown_duration
            cls._slider_cooldown_until[cookie_id] = cooldown_until
            cls._slider_consecutive_failures[cookie_id] = 0
            logger.warning(f"【{cookie_id}】滑块连续失败 {count} 次，进入 {cls._slider_cooldown_duration} 秒冷静期")
            return True
        return False

    @classmethod
    def clear_slider_failure_count(cls, cookie_id: str):
        """滑块验证成功后清零失败计数"""
        if not cookie_id:
            return
        cls._slider_consecutive_failures.pop(cookie_id, None)
        cls._slider_cooldown_until.pop(cookie_id, None)

    @classmethod
    def is_slider_in_cooldown(cls, cookie_id: str) -> bool:
        """检查是否在滑块冷静期中"""
        if not cookie_id:
            return False
        until = cls._slider_cooldown_until.get(cookie_id, 0)
        if until > time.time():
            return True
        cls._slider_cooldown_until.pop(cookie_id, None)
        return False
    ...
    @classmethod
    def get_slider_cooldown_remaining(cls, cookie_id: str) -> int:
        """获取冷静期剩余秒数"""
        if not cookie_id:
            return 0
        until = cls._slider_cooldown_until.get(cookie_id, 0)
    ...
    def _load_proxy_config(self) -> dict:
        ...
        try:
            ...
            if proxy_config and proxy_config.get('proxy_type', 'none') != 'none':
                return proxy_config

            env_proxy = self._load_proxy_config_from_env()
            if env_proxy.get('proxy_type', 'none') != 'none':
                logger.info(
                    f"【{self.cookie_id}】使用环境代理: "
                    f"{env_proxy.get('proxy_type')}://{env_proxy.get('proxy_host')}:{env_proxy.get('proxy_port')}"
                )
                return env_proxy

            return proxy_config or default_config
        except Exception as e:
            logger.warning(f"【{self.cookie_id}】加载代理配置失败: {e}，尝试使用环境代理")
            env_proxy = self._load_proxy_config_from_env()
            return env_proxy if env_proxy.get('proxy_type', 'none') != 'none' else default_config

    @staticmethod
    def _load_proxy_config_from_env() -> dict:
        proxy_url = (
            os.environ.get('DOCKER_HOST_PROXY')
        ...
        proxy_type = parsed.scheme if parsed.scheme in ('http', 'https', 'socks4', 'socks5') else 'http'
        ...
        return {
            ...
            'proxy_pass': unquote(parsed.password or '')
        }
    ...
    def begin_manual_refresh(cls, cookie_id: str, source: str = "manual_refresh") -> Dict[str, Any]:
        ...
        with cls._manual_refresh_lock:
            ...
            if existing:
                ...
                existing["expires_at"] = time.time() + cls._manual_refresh_ttl
                return {
                    "started": False,
                    "already_active": True,
            ...
            expires_at = time.time() + cls._manual_refresh_ttl
                    ...
                    if self._need_captcha_verification(res_json):
                        ...
                        try:
                            ...
                            if new_cookies_str:
                                ...
                                # 尝试第三方验证码识别服务
                                try:
                                    from utils.captcha_solver import captcha_solver as _solver
                                    if _solver.enabled:
                                        logger.info(f"【{self.cookie_id}】尝试调用第三方验证码服务破解滑块...")
                                        solve_result = await _solver.solve_slider(verification_url)
                                        if solve_result and solve_result.get('token'):
                                            logger.success(f"【{self.cookie_id}】第三方验证码服务破解成功")
                                    else:
                                        logger.info(f"【{self.cookie_id}】第三方验证码服务未配置")
                                except Exception as solver_e:
                                    logger.warning(f"【{self.cookie_id}】调用第三方验证码服务异常: {self._safe_str(solver_e)}")
    ...
    async def _handle_captcha_verification(self, res_json: dict) -> str:
        ...
        try:
            ...
            # 检查滑块冷静期
            if XianyuLive.is_slider_in_cooldown(self.cookie_id):
                remaining = XianyuLive.get_slider_cooldown_remaining(self.cookie_id)
                logger.warning(f"【{self.cookie_id}】滑块冷静期中，剩余 {remaining} 秒，跳过滑块验证")
                return None
            ...
            try:
                ...
                if success and cookies:
                    ...
                    try:
                        ...
                        XianyuLive.clear_password_login_failure_backoff(self.cookie_id)
                        XianyuLive.clear_slider_failure_count(self.cookie_id)
                        logger.info(f"【{self.cookie_id}】滑块验证成功后，已清理密码登录失败退避和滑块连续失败计数")
                    ...
                    try:
                        ...
                        if _solver.enabled:
                            ...
                            solve_result = await _solver.solve_slider(verification_url)
                            if solve_result and solve_result.get('token'):
                                logger.success(f"【{self.cookie_id}】第三方验证码服务破解成功，获取到token")
                            else:
                                logger.warning(f"【{self.cookie_id}】第三方验证码服务破解失败: {_solver.get_last_error()}")
                        else:
                            logger.info(f"【{self.cookie_id}】第三方验证码服务未配置，跳过远程破解")
                    except Exception as solver_e:
                        logger.warning(f"【{self.cookie_id}】调用第三方验证码服务异常: {self._safe_str(solver_e)}")

                    # 记录滑块连续失败，触发冷静期时发送通知
                    cooldown_triggered = XianyuLive.record_slider_failure(self.cookie_id)
                    if cooldown_triggered:
                        cooldown_minutes = XianyuLive._slider_cooldown_duration // 60
                        cooldown_msg = (
                            f"【账号风控提醒】{self.cookie_id}\n"
                            f"滑块验证连续失败 {XianyuLive._slider_cooldown_threshold} 次\n"
                            f"已进入 {cooldown_minutes} 分钟冷静期，期间跳过滑块验证\n"
                            f"请及时检查账号状态或手动处理 Cookie"
                        )
                        logger.warning(cooldown_msg)
                        await self.send_token_refresh_notification(
                            cooldown_msg,
                            "slider_cooldown_activated"
                        )
    ...
    async def _try_password_login_refresh(
        ...
        try:
            ...
            if result:
                ...
                if backoff_reason == 'qr_verify_timeout':
                    XianyuLive._slider_cooldown_until[self.cookie_id] = time.time() + backoff_seconds
                    await self.send_token_refresh_notification(
                        f"【二维码验证超时】{self.cookie_id}\n"
                        f"二维码验证等待 180 秒后超时，已进入 {backoff_seconds // 60} 分钟冷静期\n"
                        f"请尽快手动扫码完成验证或检查账号状态",
                        "qr_verify_timeout"
                    )
                if refresh_risk_log_id:
                    self._update_risk_log(
    ...
    async def send_notification(self, send_user_name: str, send_user_id: str, send_message: str, item_id: str = None, chat_id: str = None):
        ...
        try:
            ...
            try:
                ...
                if chat_id:
                    await self._notify_buyer_bargain_reply(chat_id, send_user_name, send_message, item_id)
    ...
    async def _notify_buyer_bargain_reply(self, chat_id: str, seller_name: str, reply_message: str, item_id: str = None):
        """检测卖家回复是否为主动议价的回复，并发送通知"""
        try:
            from db_manager import db_manager
            rows = db_manager.execute_query(
                "SELECT id, user_id, item_title, account_id FROM buyer_bargains WHERE chat_id = ? AND status = 'sent' AND (has_reply IS NULL OR has_reply = 0)",
                (chat_id,)
            )
            if not rows:
                return
            ...
            for row in rows:
                bargain_id = row[0]
                user_id = row[1]
                item_title = row[2] or '未知商品'
                account_id = row[3] or self.cookie_id

                now = time.strftime('%Y-%m-%d %H:%M:%S')
                with db_manager.lock:
                    cursor = db_manager.conn.cursor()
                    cursor.execute(
                        "UPDATE buyer_bargains SET has_reply = 1, reply_content = ?, replied_at = ?, updated_at = ? WHERE id = ?",
                        (reply_message, now, now, bargain_id)
                    )
                    db_manager.conn.commit()

                logger.info(f"✅ 收到卖家对议价的回复: bargain_id={bargain_id}, item={item_title}, seller={seller_name}")

                reply_notification = render_notification_template(
                    'message',
                    account_id=account_id,
                    buyer_name=f"卖家{seller_name}(议价回复)",
                    buyer_id=chat_id,
                    item_id=item_id or '未知',
                    chat_id=chat_id,
                    message=f"[卖家回复议价] 「{item_title}」:{reply_message}",
                    time=now,
                )
                await dispatch_account_notifications(
                    account_id,
                    reply_notification,
                    title='卖家回复议价通知',
                    notification_type='message',
            ...
            logger.error(f"处理卖家议价回复通知失败: {self._safe_str(e)}")
    ...
    async def send_msg_once(self, toid, item_id, text):
        ...
        Returns:
            (成功标志, chat_id) — chat_id 在发送成功时返回，失败时返回 None
        """
        headers = self._build_websocket_headers()
        ...
        try:
            async with websockets.connect(
                ...
                success, cid = await self._handle_websocket_connection(websocket, toid, item_id, text)
                if success:
                    logger.info(f"【{self.cookie_id}】单次发送消息成功, chat_id={cid}")
                    return (True, cid)
                else:
            ...
            if "extra_headers" in error_msg:
                ...
                async with websockets.connect(
                    ...
                    success, cid = await self._handle_websocket_connection(websocket, toid, item_id, text)
                    if success:
                        logger.info(f"【{self.cookie_id}】单次发送消息成功(兼容模式), chat_id={cid}")
                        return (True, cid)
                    else:
                        raise Exception("消息发送失败")
            else:
        ...
        if proxy_url:
            ...
            try:
                ...
                if socks_type:
                    ...
                    # 通过代理建立真实 socket，再交给 websockets 处理 TLS / WebSocket 握手
                    proxy_sock = proxy.connect(
                        dest_host=dest_host,
                        dest_port=dest_port,
                        timeout=10
                    )
    ...
    async def _handle_websocket_connection(self, websocket, toid, item_id, text):
        """处理WebSocket连接的具体逻辑，返回 (成功标志, chat_id)"""
        try:
            await self.init(websocket)
            await self.create_chat(websocket, toid, item_id)
